    ...
    def run(self):
        ...
        if self.neuron_type == 'LIF':
            for self.timepoint in tqdm(range(self.total_timepoints)) if self.kwargs.get('process_bar', False)\
                ...
                self.refractory +=1
                ### update potentials
                ## TODO: Improve approximation and numercial stability
                ## Numerical stability
                ...
                STDP[spikes,:] = - 1.5*self.AdjacencyMatrix[spikes,:] * self.STDP_trace.T
                ...
                # for a, b in zip([pw,pstdp, pd, pet, pr],
                #     [self.weights[0][1].item(),STDP[0][1].item(), self.dopamine.item(), self.eligibility_trace[0][1].item(), self.reward[self.timepoint].item() ]):
                #     a.append(b)
            ...
            #print(self.AdjacencyMatrix[0,:].sum(),self.AdjacencyMatrix[:,0].sum())
            for self.timepoint in tqdm(range(self.total_timepoints)) if self.kwargs.get('process_bar', False)\
            else range(self.total_timepoints):
                ...
                # n0spikes = np.where(self.spike_train[0].cpu())[0]
                # n1spikes = np.where(self.spike_train[1].cpu())[0]

                # for a, b in zip([pw,pstdp, pd, pet, pr],
                #     [self.weights[0][1].item(),STDP[0][1].item(), self.dopamine.item(), self.eligibility_trace[0][1].item(), self.reward[self.timepoint].item() ]):
                #     a.append(b)
                stdps.append(self.sd[0][1].cpu())
                if self.timepoint == self.total_timepoints-1 or self.timepoint%1000==0:
                    target_weights.append(self.weights[0][1].cpu())
                    plot_times.append(self.timepoint/1000)
                    ...
                    # axs[0].set_ylim([0,self.N])
                    axs[0].plot(stdps)
                    # axs[0].plot(n0spikes, np.ones_like(n0spikes)* -.15, 'ro', markersize = 1)
                    # axs[0].plot(n1spikes, np.ones_like(n1spikes)* .1, 'ro', markersize = 1)
                    # weights histogram
                    axs[1].hist(self.weights.cpu()[self.weights>0], bins = 100)
                    #target synapse's weight and rewarsd
                    axs[2].plot(plot_times, target_weights,)
