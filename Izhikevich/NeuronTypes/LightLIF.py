...
class NeuronGroup:
    ...
    def run(self):
        ...
        for self.time in tqdm(range(self.total_time)) if self.kwargs.get('process_bar', False)\
            else range(self.total_time):
            self.spike_train = torch.zeros((self.N, 1000), dtype= torch.bool)
            for self.timestep in range(1000):
                self.refractory +=1
                ### update potentials
                ...
                ### Spike train
                self.spike_train[:,self.timestep] = spikes
