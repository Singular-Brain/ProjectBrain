...
class NetworkPanel:
    ...
    def _update_hover_tooltips(self, timepoint):
        if self.group.save_history:
            ...
            neurons_current = dict([(i[0], i[1]) for i in enumerate(self.group.current_history[:,timepoint].cpu().numpy())])
    ...
    def _update_network(self, timepoint):
        ...
        #Update Spikes
        spiked_neurones = dict([(i[0], i[1]) for i in enumerate((self.group.spike_train[:,timepoint].cpu().numpy()).astype(str))])
...
class NetworkPanelData:
    ...
    def __init__(self, DataFile, input_neurons, file_path, verbose = False):
        self.data = np.load(DataFile, allow_pickle=True)[()]
        self.file_path = file_path
        self.input_neurons = input_neurons
        self.verbose = verbose
        self.run = 0
        rows, cols = np.where(self.data['AdjacencyMatrix'] == 1)
        edges = zip(rows.tolist(), cols.tolist())
        self.graph = nx.DiGraph()
        self.graph.add_edges_from(edges)
        self.pos = self._set_pos(self.graph, self.input_neurons)
        self._update_hover_tooltips(0)
        self.plot = figure(
            plot_width=1000, plot_height=480,
            tooltips = self.HOVER_TOOLTIPS,
            tools="pan,wheel_zoom,save,reset", active_scroll='wheel_zoom',
            x_range=Range1d(-.1, sorted(self.pos.values())[-1][0]+0.1),
            y_range=Range1d(0, 1), title='Neuron Group')
        self._update_network(0,0)
        #Run slider
        self.run_slider = Slider(start=1, end=len(self.data['runs']), value=1, step=1, title="run")
        self.run_slider.on_change('value', self._runs_slider_update)
        #Timepoint slider
    ...
    def _update_hover_tooltips(self, timepoint):
        if self.data['save_history']:
            neurons_potential = dict([(i[0], i[1]) for i in enumerate(self.data['runs'][self.run]['potential_history'][:,timepoint].cpu().numpy())])
            nx.set_node_attributes(self.graph, name='neurons_potential', values=neurons_potential) 
            neurons_current = dict([(i[0], i[1]) for i in enumerate(self.data['runs'][self.run]['current_history'][:,timepoint].cpu().numpy())])
            nx.set_node_attributes(self.graph, name='neurons_current', values=neurons_current)
            self.HOVER_TOOLTIPS = [ ("Neuron", "@index"),
                                    ("Potential", '@neurons_potential'),
                                    ("Current", '@neurons_current'),
    ...
    def _animate_update(self):
        self.timepoint = self.timepoint_slider.value + 1
        if self.timepoint > int(self.data['total_time']/ self.data['dt']) -1 :
            self.timepoint = 0
    ...
    def _runs_slider_update(self, attrname, old, new):
        ...
        if self.verbose:
            pprint(self.data['runs'][self.run])
        self._update_network(self.run, 0)

    def _timepoint_slider_update(self, attrname, old, new):
        timepoint = self.timepoint_slider.value
        self._update_network(self.run, timepoint)

    def _update_network(self, run, timepoint):
        self._update_hover_tooltips(timepoint)
        #Update Spikes
        spiked_neurones = dict([(i[0], i[1]) for i in enumerate((self.data['runs'][self.run]['spike_train'][:,timepoint].cpu().numpy()).astype(str))])
        nx.set_node_attributes(self.graph, name='spiked_neurons', values=spiked_neurones) 
        #
        elements = np.nditer(self.data['runs'][self.run]['weights'], flags=['multi_index'])
        excitetory_inhibitory = {(elements.multi_index[0], elements.multi_index[1]):  str(np.sign(i)) for i in elements if i !=0.0}
        nx.set_edge_attributes(self.graph, name='excitetory_inhibitory', values=excitetory_inhibitory)
        #Add network graph from networkx
        self.network_graph = from_networkx(self.graph, self.pos, scale=10, center=(0, 0))
